...
def review_complaint(fields: dict[str, Any], existing_records: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    comparable_records = [
        {"complaint_id": item.get("complaint_id"), "fields": item.get("fields", {})}
        for item in existing_records
    ]
    prompt = (
        "Review this pharmaceutical complaint and return JSON only with exactly these keys: "
        "completeness, duplicate_detection, summary, risk_classification, root_cause_recommendation, "
        "capa_recommendation. completeness must contain score (0-100), missing_fields (array), "
        "and explanation. duplicate_detection must contain is_duplicate (boolean), matching_complaint_ids "
        "(array), and explanation. risk_classification must contain severity, priority, and rationale. "
        ...
        f"Existing complaint records for duplicate comparison: {json.dumps(comparable_records)}"
    )
    result = parse_json_response(model.invoke(prompt).content)
    required = {"completeness", "duplicate_detection", "summary", "risk_classification", "root_cause_recommendation", "capa_recommendation"}
